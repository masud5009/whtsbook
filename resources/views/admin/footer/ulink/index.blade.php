...
@section('content')
    <div class="page-header">
        <h4 class="page-title">{{ __('Userful Links') }}</h4>
                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-5 mb-3 mb-lg-0">
                            <div class="card-title d-inline-block">{{ __('Userful Links') }}</div>
                        </div>
                        <div class="col-lg-2">
                            @if (!empty($langs))
                        ...
                        <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                            <a href="#" class="btn btn-primary float-right btn-sm" data-toggle="modal"
                                data-target="#createModal"><i class="fas fa-plus"></i> {{ __('Add Userful Link') }}</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                                        ...
                                        <tbody>
                                            @foreach ($aulinks as $key => $aulink)
                                                <tr>
                                                    ...
                                                    <td>{{ $aulink->url }}</td>
                                                    <td>
                                                        <a class="mb-1 btn btn-secondary btn-sm editbtn" href="#editModal"
                                                            data-toggle="modal" data-ulink_id="{{ $aulink->id }}"
                                                            data-name="{{ $aulink->name }}"
                                                            data-url="{{ $aulink->url }}">
                                                        ...
                                                        <form class="deleteform d-inline-block"
                                                            action="{{ route('admin.ulink.delete') }}" method="post">
                                                            @csrf
                                                            <input type="hidden" name="ulink_id"
                                                                value="{{ $aulink->id }}">
                                                            <button type="submit"
                                                                class="mb-1 btn btn-danger btn-sm deletebtn">
                                                                <span class="btn-label">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Add Userful Link') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="ajaxForm" class="modal-form create" action="{{ route('admin.ulink.store') }}"
                        method="POST">
                        @csrf
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Edit Userful Link') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="ajaxEditForm" action="{{ route('admin.ulink.update') }}" method="POST">
                        @csrf
                        <input id="in_ulink_id" type="hidden" name="ulink_id" value="">
                        <div class="form-group">
                            <label for="">{{ __('Name') }} <span class="text-danger">**</span></label>
                        ...
                        <div class="form-group">
                            <label for="">{{ __('URL') }} <span class="text-danger">**</span></label>
                            <input id="in_url" class="form-control ltr" name="url"
