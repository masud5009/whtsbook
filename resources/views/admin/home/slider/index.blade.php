...
@section('content')
    <div class="page-header">
        <h4 class="page-title">{{ __('Hero Section Sliders') }}</h4>
        <ul class="breadcrumbs">
            ...
            <li class="nav-item">
                <a href="#">{{ __('Hero Section Sliders') }}</a>
            </li>
        </ul>
    </div>
    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-5 mt-2 mt-lg-0">
                            <a href="#" class="btn btn-primary float-right btn-sm" data-toggle="modal"
                                data-target="#createModal"><i class="fas fa-plus"></i> {{ __('Add Slider') }}</a>
                            <button class="btn btn-danger float-right btn-sm mr-2 d-none bulk-delete"
                                data-href="{{ route('admin.slider.bulk.delete') }}"><i class="flaticon-interface-5"></i>
                                {{ __('Delete') }}</button>
                        </div>
                    </div>
                </div>
                                        ...
                                        <tbody>
                                            @foreach ($sliders as $slider)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <img src="{{ asset('assets/front/img/hero_slider/' . $slider->img) }}"
                                                            class="max-w-130" alt="slider">
                                                    </td>
                                                    <td>
                                                        {{ strlen($slider->title) > 40 ? mb_substr($slider->title, 0, 40, 'UTF-8') . '...' : $slider->title }}
                                                    </td>
                                                    <td>
                                                        {{ strlen($slider->subtitle) > 40 ? mb_substr($slider->subtitle, 0, 40, 'UTF-8') . '...' : $slider->subtitle }}
                                                    </td>
                                                    <td>{{ $slider->serial_number }}</td>
                                                    <td>
                                                        <a class="btn btn-secondary btn-sm editbtn mb-1" href="#editModal"
                                                            data-toggle="modal" data-slider_id="{{ $slider->id }}"
                                                            data-title="{{ $slider->title }}"
                                                            data-subtitle="{{ $slider->subtitle }}"
                                                            data-description="{{ $slider->description }}"
                                                            data-btn_name="{{ $slider->btn_name }}"
                                                            data-btn_url="{{ $slider->btn_url }}"
                                                            data-serial_number="{{ $slider->serial_number }}"
                                                            data-img="{{ $slider->img ? asset('assets/front/img/hero_slider/' . $slider->img) : asset('assets/admin/img/noimage.jpg') }}">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ajaxForm" class="modal-form create" action="{{ route('admin.slider.store') }}"
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    ...
                                    <p id="err_img" class="mb-0 text-danger em"></p>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <span class="text-info">
                                {{ __('If you wrap the text with') }}
                                <b><code>&lt;span&gt;&lt;/span&gt;</code></b>
                                {{ __('it will appear in italic.') }}
                            </span>
                        </div>
                        <div class="form-group">
                            <label for="">{{ __('Description') }} <span class="text-danger">**</span></label>
                            <textarea class="form-control" name="description" rows="3" placeholder="{{ __('Enter description') }}"></textarea>
                            <p id="err_description" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                            <label for="">{{ __('Button Name') }}</label>
                            <input type="text" class="form-control" name="btn_name" value=""
                                placeholder="{{ __('Enter button name') }}">
                            <p id="err_btn_name" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                            <label for="">{{ __('Button URL') }}</label>
                            <input type="url" class="form-control ltr" name="btn_url" value=""
                                placeholder="{{ __('Enter button url') }}">
                            <p id="err_btn_url" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                            ...
                            <p class="text-warning">
                                <small>{{ __('The higher the serial number is, the later the slider will be shown.') }}</small>
                            </p>
                        </div>
                    </form>
                </div>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ajaxEditForm" action="{{ route('admin.slider.update') }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <input id="in_slider_id" type="hidden" name="slider_id" value="">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <div class="col-12 mb-2 pl-0">
                                    ...
                                    <div class="col-md-12 showImage2 mb-3 pl-0">
                                        <img id="in_img" src="{{ asset('assets/admin/img/noimage.jpg') }}"
                                    ...
                                    <p id="editErr_img" class="mb-0 text-danger em"></p>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="">{{ __('Subtitle') }} <span class="text-danger">**</span></label>
                            <input id="in_subtitle" type="text" class="form-control" name="subtitle" value=""
                                placeholder="{{ __('Enter subtitle') }}">
                        ...
                        <div class="form-group">
                            <label for="">{{ __('Description') }} <span class="text-danger">**</span></label>
                            <textarea id="in_description" class="form-control" name="description" rows="3"
                                placeholder="{{ __('Enter description') }}"></textarea>
                            <p id="editErr_description" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                            <label for="">{{ __('Button Name') }}</label>
                            <input id="in_btn_name" type="text" class="form-control" name="btn_name" value=""
                                placeholder="{{ __('Enter button name') }}">
                            <p id="editErr_btn_name" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                            <label for="">{{ __('Button URL') }}</label>
                            <input id="in_btn_url" type="url" class="form-control ltr" name="btn_url"
                                value="" placeholder="{{ __('Enter button url') }}">
                            <p id="editErr_btn_url" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                            <label for="">{{ __('Serial Number') }} <span class="text-danger">**</span></label>
